...
public class DatabaseManager {
    ...
    public boolean clearDb(User user) {
        ...
        String deleteDragonsSQL = "DELETE FROM dragon WHERE owner_username = ?";
        String deletePersonsSQL = "DELETE FROM person WHERE id IN (SELECT killer_id FROM dragon WHERE owner_username = ?)";
        String deleteCoordinatesSQL = "DELETE FROM coordinates WHERE id IN (SELECT coordinates_id FROM dragon WHERE owner_username = ?)";
        String deleteLocationsSQL = "DELETE FROM location WHERE id IN (SELECT location_id FROM person WHERE id IN (SELECT killer_id FROM dragon WHERE owner_username = ?))";
    ...
    public int addDragon(Dragon dragon, User user) {
        ...
        try {
            ...
            String query = "INSERT INTO dragon (name, owner_username, coordinates_id, creation_date, age, speaking, color, character, killer_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING id";
            PreparedStatement preparedStatement = connection.prepareStatement(query);
            preparedStatement.setString(1, dragon.getName());
            preparedStatement.setString(2, user.getUsername());
            preparedStatement.setInt(3, coordinatesId);
            preparedStatement.setTimestamp(4, Timestamp.valueOf(dragon.getCreationDate()));
            ...
            preparedStatement.setObject(7, dragon.getColor().toString(), Types.OTHER);

            if (dragon.getCharacter() != null)
                preparedStatement.setObject(8, dragon.getCharacter().toString(), Types.OTHER);
            else
        ...
        try {
            ...
            if (dragon.getKiller() != null)
                if (dragon.getKiller().getId() != -1) {
                    ...
                    preparedStatement.setInt(6, dragon.getKiller().getId());
                }
                else
    ...
    public boolean removeDragon(int dragonId, User user) {
        String checkOwnerSQL = "SELECT owner_username FROM dragon WHERE id = ?";
        ...
        try (Connection connection = connect()) {
            ...
            try (PreparedStatement pstmt = connection.prepareStatement(checkOwnerSQL)) {
                pstmt.setInt(1, dragonId);
                ResultSet resultSet = pstmt.executeQuery();
                if (resultSet.next()) {
                    if (!Objects.equals(resultSet.getString("owner_username"), user.getUsername())) {
                        return false;
                    }
                } else {
