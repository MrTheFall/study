package common.utils;

public interface Commands {
    String HELP = "help";
    String INFO = "info";
    String SHOW = "show";
    String SORT = "sort";
    String ADD = "add";
    String AVERAGE_OF_AGE = "average_of_age";
    String GROUP_COUNTING_BY_CREATION_DATE = "group_counting_by_creation_date";
    String UPDATE = "update";
    String INSERT = "insert";
    String PRINT_UNIQUE_COLOR = "print_unique_color";
    String REMOVE_BY_ID = "remove_by_id";
    String CLEAR = "clear";
    String EXIT = "exit";
    String EXECUTE_SCRIPT = "execute_script";
    String HEAD = "head";
    String ADD_IF_MAX = "add_if_max";
    String ADD_IF_MIN = "add_if_min";
    String SUM_OF_PRICE = "sum_of_price";
    String FILTER_BY_PRICE = "filter_by_price";
    String FILTER_CONTAINS_PART_NUMBER = "filter_contains_part_number";
}