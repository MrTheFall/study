...
public class GroupCountingByCreationDate extends Command {
    private static final long serialVersionUID = 4141241274298L;
    ...
    public Response apply(Map<ArgumentType, Object> args) {
        ...
        String message = "Группировка коллекции по дате создания:\n";
        for (Map.Entry<LocalDateTime, List<Dragon>> entry : groups.entrySet()) {
            LocalDateTime date = entry.getKey();
            List<Dragon> dragons = entry.getValue();
            message += "Дата создания: " + date + " | Количество драконов: " + dragons.size() + "\n";
        }

        return new Response(true, message, groups);
    }
